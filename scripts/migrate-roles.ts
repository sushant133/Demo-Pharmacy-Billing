/**
 * One-off migration: fold the old pharmacist and cashier roles into admin.
 *
 *   npx tsx scripts/migrate-roles.ts          report what would change
 *   npx tsx scripts/migrate-roles.ts --apply  write the changes
 *
 * A pharmacy this size is run from one counter by one person, so the system
 * now has a single role that holds every permission. Accounts created under
 * the old split still carry "pharmacist" or "cashier", which the User schema's
 * enum no longer accepts - the next save of such a document would fail
 * validation. This promotes them in place.
 *
 ...
 * history recorded against them, and simply gain the screens they were
...
/**
 * Read through the driver rather than the model: Mongoose would have to cast
 * these documents against an enum that no longer lists their role.
 */
function users() {
...
async function promoteStaleRoles(): Promise<void> {
  ...
  if (stale.length === 0) {
    console.log("  every account is already an admin.");
    return;
  }
  ...
  const result = await users().updateMany(filter, { $set: { role: "admin" } });
