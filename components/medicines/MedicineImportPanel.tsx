...
import { IMPORT_TEMPLATE_HEADER } from "@/lib/medicine-import";
 ...
 * thing being imported visible rather than a filename.
 */
...
export function MedicineImportPanel({
  ...
  return (
    <SlideOver
      title={done ? "Import finished" : "Bulk import medicines"}
      description={
        done
          ? undefined
          : "Paste rows from a spreadsheet, or choose a CSV file. Nothing is saved until you have seen what it will do."
      }
      onClose={close}
      footer={
        done ? (
          ...
          <div className="flex gap-2">
            {report ? (
              ...
              >
                {busy
                  ? "Importing…"
                  : `Import ${report.willCreate} medicine${report.willCreate === 1 ? "" : "s"}`}
              </button>
            ) : (
              <button
                type="button"
